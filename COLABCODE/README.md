# Colab Bundle for X-GATE

This folder is the Google Colab-ready copy of the X-GATE experiment code.

Start with [COLAB_SETUP.md](./COLAB_SETUP.md) for the exact Google Drive layout,
install command, and run commands.
...
### 3. Closing Deployment / Robustness Claims from Saved Checkpoints

After the canonical multi-run experiment finishes, compute the missing
deployment-side evidence (dynamic INT8 post-training evaluation, CPU latency,
artifact size, held-out adversarial metrics, and Logical Drift) from the saved
checkpoints:

```bash
python run_claim_closure.py --results-root results/multirun
```

If your canonical run used a custom results directory, point `--results-root`
