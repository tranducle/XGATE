...
from run_all_experiments import (
    ...
    LEARNING_RATE, WEIGHT_DECAY, KD_ALPHA, KD_TEMP, DRIFT_SAMPLES,
)
...
def main():
    ...
    train_loader, val_loader, meta = get_dataloaders(batch_size=BATCH_SIZE)
    logger.info(f" Data: {meta['train_samples']:,} train, {meta['eval_samples']:,} val (batch={BATCH_SIZE})")
    ...
    for run_idx in range(N_RUNS):
        ...
        for config_name, config in ABLATION_CONFIGS.items():
            ...
            try:
                ...
                result = train_ablation_variant(
                    ...
                    device=device, config_name=config_name,
                ...
                try:
                    spearman_rho, drift = evaluate_logical_drift(
                        teacher, student, val_loader, device, num_samples=DRIFT_SAMPLES
                    )
                except Exception:
                ...
                try:
                    adv = evaluate_adversarial_fpr(
                        ...
                        epsilon=EPSILON, num_samples=2000,
                    )
                ...
                # Latency profiling
                lat = evaluate_latency(student, device)

                run_metrics[config_name] = {
                    "f1_macro":        result.get("best_f1_macro", vm.get("f1_macro", 0.0)),
        ...
        # Save per-run
        with open(run_dir / "ablation_metrics.json", "w") as f:
