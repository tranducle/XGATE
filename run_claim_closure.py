...
DEFAULT_RESULTS_ROOT = Path("results/multirun")
...
MODEL_FACTORIES = {
    ...
    "X-GATE_TinyStudent": lambda: TinySecurityBERT(
        num_classes=NUM_CLASSES, input_features=INPUT_FEATURES
    ),
    "Standard_KD_TinyStudent": lambda: TinySecurityBERT(
        num_classes=NUM_CLASSES, input_features=INPUT_FEATURES
    ),
}
...
def main():
    ...
    for run_index in range(min(args.n_runs, len(SEEDS))):
        ...
        teacher_path = run_dir / "Vanilla_SecurityBERT_Teacher_best.pth"
        ...
        for student_name in ("X-GATE_TinyStudent", "Standard_KD_TinyStudent"):
